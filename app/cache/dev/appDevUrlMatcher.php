...
class appDevUrlMatcher extends Symfony\Bundle\FrameworkBundle\Routing\RedirectableUrlMatcher
{
    ...
    public function match($pathinfo)
    {
        ...
        if (0 === strpos($pathinfo, '/admin')) {
            ...
            if (0 === strpos($pathinfo, '/admin/managing-')) {
                // BloggerAdminBundle_delete_userInfo
                if (0 === strpos($pathinfo, '/admin/managing-users-info/deleting') && preg_match('#^/admin/managing\\-users\\-info/deleting/(?P<userId>\\d+)$#s', $pathinfo, $matches)) {
                    if (!in_array($this->context->getMethod(), array('GET', 'POST', 'HEAD'))) {
                        $allow = array_merge($allow, array('GET', 'POST', 'HEAD'));
                        goto not_BloggerAdminBundle_delete_userInfo;
                    }

                    return $this->mergeDefaults(array_replace($matches, array('_route' => 'BloggerAdminBundle_delete_userInfo')), array (  '_controller' => 'Blogger\\AdminBundle\\Controller\\UserController::deleteAction',));
                }
                not_BloggerAdminBundle_delete_userInfo:
            ...
            if (0 === strpos($pathinfo, '/admin/add-new-category')) {
                // BloggerAdminBundle_add_new_category
                if ($pathinfo === '/admin/add-new-category') {
                    if (!in_array($this->context->getMethod(), array('GET', 'POST', 'HEAD'))) {
                        $allow = array_merge($allow, array('GET', 'POST', 'HEAD'));
                        goto not_BloggerAdminBundle_add_new_category;
                    }

                    return array (  '_controller' => 'Blogger\\AdminBundle\\Controller\\CategoryController::addCategoryAction',  '_route' => 'BloggerAdminBundle_add_new_category',);
                }
                not_BloggerAdminBundle_add_new_category:

                // BloggerAdminBundle_submit_adding_category
                if ($pathinfo === '/admin/add-new-category/saving') {
                    if (!in_array($this->context->getMethod(), array('GET', 'POST', 'HEAD'))) {
                        $allow = array_merge($allow, array('GET', 'POST', 'HEAD'));
                        goto not_BloggerAdminBundle_submit_adding_category;
                    }

                    return array (  '_controller' => 'Blogger\\AdminBundle\\Controller\\CategoryController::submitAddingAction',  '_route' => 'BloggerAdminBundle_submit_adding_category',);
                }
                not_BloggerAdminBundle_submit_adding_category:
